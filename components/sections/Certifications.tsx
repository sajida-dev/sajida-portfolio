...
export default function Certifications() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      default:
        return <BookmarkCheck className="w-4 h-4 text-purple-600" />;
    }
  };

  return (
    <section id="certifications" className="py-24 bg-slate-50/50 border-b border-slate-200/80">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 text-[12px] font-bold uppercase tracking-[0.06em] rounded-lg transition-all duration-200 flex items-center gap-2 border ${
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCertifications.map((cert, idx) => (
            <div
              key={idx}
              className="bg-white rounded-xl border border-slate-200/80 p-5 flex flex-col justify-between hover:border-purple-300 hover:shadow-md transition-all duration-300 group"
            >
              <div>
                {/* Certificate Image Frame */}
                <div 
                  onClick={() => setSelectedImage(cert.image)}
                  className="w-full aspect-[16/10] rounded-lg border border-slate-200 bg-slate-50 overflow-hidden relative cursor-pointer shadow-xs mb-4.5 group/img"
                >
                  <img
                    src={cert.image}
                    alt={cert.title}
                ...
                {/* Title & Issuer Info */}
                <h3 className="font-sans font-bold text-slate-900 text-[14px] md:text-[15px] leading-snug group-hover:text-purple-600 transition-colors">
                  {cert.title}
                </h3>
              ...
              <div className="mt-4 pt-3.5 border-t border-slate-100 flex items-center justify-between">
                <span className="px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.08em] rounded border bg-purple-50/60 text-purple-700 border-purple-100">
                  {cert.category}
                </span>
              </div>
            </div>
