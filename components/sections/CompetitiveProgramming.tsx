...
import { Trophy, ExternalLink, Code2, FileText, CheckCircle, ZoomIn, X } from "lucide-react";
...
export default function CompetitiveProgramming() {
  ...
  const getBrandIcon = (name: string) => {
    switch (name.toLowerCase()) {
      ...
      default:
        return <Code2 className="w-4 h-4 text-purple-600 shrink-0" />;
    }
  };
    ...
    <section id="skills" className="py-24 bg-white border-b border-slate-200/80">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* Left Side: Contest Log (8 Columns) */}
          <div className="lg:col-span-8 space-y-6">
            <h3 className="font-sans font-bold text-slate-900 text-[18px] mb-4 flex items-center gap-2">
              <Trophy className="w-5 h-5 text-amber-500" /> Contest Logs & Certificates
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {portfolioData.competitiveProgramming.competitions.map((competition, idx) => (
                <div 
                  key={idx} 
                  className="bg-slate-50/60 border border-slate-200/80 rounded-xl p-6 flex flex-col justify-between hover:border-purple-300 hover:shadow-md transition-all duration-300"
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="font-mono text-[9px] font-bold uppercase tracking-[0.08em] px-2.5 py-0.5 rounded-md bg-purple-50 border border-purple-100 text-purple-700 shrink-0">
                        {competition.badge || "Participant"}
                      </span>
                    </div>

                    {/* Certificate Image or PDF Certificate */}
                    {competition.image ? (
                      <div 
                        onClick={() => setSelectedImage(competition.image || null)}
                        className="w-full aspect-[16/10] rounded-lg border border-slate-200 overflow-hidden relative shadow-xs bg-white cursor-pointer group/img"
                      >
                        <img 
                          src={competition.image} 
                          alt={`${competition.name} Certificate`}
                          className="w-full h-full object-contain p-1 transition-transform duration-500 group-hover/img:scale-[1.03]"
                        />
                        <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2">
                          <span className="bg-white/95 text-slate-800 text-[11px] font-bold px-3 py-1.5 rounded-md shadow flex items-center gap-1.5">
                            <ZoomIn className="w-3.5 h-3.5 text-purple-600" /> View Certificate
                          </span>
                        </div>
                      </div>
                    ) : competition.url ? (
                      <div className="w-full aspect-[16/10] rounded-lg bg-white border border-slate-250 flex flex-col items-center justify-center p-4 relative overflow-hidden select-none shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)]">
                        ...
                        <div className="flex flex-col items-center justify-center text-center space-y-2 z-10">
                          <div className="relative">
                            ...
                            <CheckCircle className="w-4.5 h-4.5 text-purple-600 absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 border border-purple-100" />
                          </div>
                  ...
                  {(competition.image || competition.url) && (
                    <div className="mt-5 pt-4 border-t border-slate-200/60">
                      {competition.image ? (
                        <button
                          onClick={() => setSelectedImage(competition.image || null)}
                          className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.06em] rounded-lg bg-white hover:bg-slate-50 text-slate-700 hover:text-purple-600 border border-slate-250 hover:border-purple-300 shadow-xs transition-all text-center cursor-pointer font-sans"
                        >
                          View Certificate <ZoomIn className="w-3.5 h-3.5 text-purple-600" />
                        </button>
                      ) : (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.06em] rounded-lg bg-white hover:bg-slate-50 text-slate-700 hover:text-purple-600 border border-slate-250 hover:border-purple-300 shadow-xs transition-all text-center"
                        >
              ...
              <div className="flex flex-col gap-3">
                {portfolioData.competitiveProgramming.profiles.map((profile, idx) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 hover:border-purple-300 hover:bg-purple-50/40 text-[12px] font-bold text-slate-700 hover:text-purple-600 transition-all group"
                  >
                    <div className="shrink-0 w-8 h-8 flex items-center justify-center bg-slate-50 border border-slate-100 rounded group-hover:bg-purple-100/50 transition-colors">
                      {getBrandIcon(profile.name)}
                    </div>
                    <span>{profile.name}</span>
                    <ExternalLink className="w-3.5 h-3.5 ml-auto text-slate-400 group-hover:text-purple-600" />
                  </a>
                ))}
              </div>
            </div>
